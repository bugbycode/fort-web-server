...
public class RoleController {
	...
	public String insert(@ModelAttribute("role") Role role) {
		...
		JSONObject result = dataRequestService.modify(resourceServer.getUserServerUrl() + AppConfig.ROLE_INSERT_PATH, param);
		if(result.getIntValue("code") == 0) {
			...
			log.setModuleId(result.getString("roleId"));
			log.setModule(LogConfig.ROLE_MODULE);
	...
	public String update(@ModelAttribute("role") Role role) {
		...
		log.setModuleId(String.valueOf(roleId));
		log.setModule(LogConfig.ROLE_MODULE);
		log.setLevel(LogConfig.USER_LEVEL);
		param.clear();
		param.put("jsonStr", JSONObject.toJSONString(log));
